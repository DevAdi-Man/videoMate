...
const createComunity = asyncHandler(async (req, res) => {
  ...
  if (!comunity) {
    return next(new ApiError(400, "comunity not created"));
  }
...
const getUserComunity = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, getComunity, "fetched comunity successfully"));
});
...
const updateComunity = asyncHandler(async (req, res) => {
  ...
  if (!isValidObjectId(comunityId)) {
    return next(new ApiError(404, "comunity not found"));
  }
  ...
  if (!comunity) {
    return next(new ApiError(400, "comunity not updated"));
  }

  return res
    .status(200)
    .json(new ApiResponse(200, comunity, "comunity updated successfully"));
});
...
const deleteComunity = asyncHandler(async (req, res) => {
  ...
  if (!comunity) {
    return next(new ApiError(400, "comunity not deleted"));
  }

  if (comunity?.owner.toString() !== req.user._id.toString()) {
    return next(
      new ApiError(400, "you are not authorized to delete this comunity")
    );
  }
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, deletedComunity, "comunity deleted successfully")
    );
});
