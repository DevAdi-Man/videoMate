...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  if (!videoFile && !thumbnail) {
    return next(new ApiError(500, "video and thumbnail field are required"));
  }
  ...
  if (!videos) {
    return next(
      new ApiError(401, "something went worng while publishing video")
    );
  }
...
const getVideoById = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    return next(new ApiError(400, "please login "));
  }
  ...
  if (!view) {
    return next(new ApiError(400, "somethin went wrong while updating views"));
  }
  return res
    .status(200)
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  if (!videoId?.trim()) {
    return next(new ApiError(400, "videoId is missing"));
  }
  ...
  if (!isVideo) {
    return next(new ApiError(404, "video is not find"));
  }

  const thumbnailFilePath = req.file?.path;

  if (!thumbnailFilePath) {
    return next(new ApiError(400, "thumbnail file is missig"));
  }
  ...
  if (!thumbnail) {
    return next(
      new ApiError(400, "somethin went worng while uploading thumbnail")
    );
  }
...
const togglePublishStatus = asyncHandler(async (req, res) => {
  ...
  if (!videoId) {
    return next(new ApiError(404, "video Id is missing"));
  }
  ...
  return res.status(200).json(new ApiResponse(200, video, "video toggled"));
});
