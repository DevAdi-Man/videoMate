...
const toggleCommentLike = asyncHandler(async (req, res) => {
  ...
  if (!commentId) {
    return next(new ApiError(404, "comment id  is missing"));
  }
...
const toggleTweetLike = asyncHandler(async (req, res) => {
  ...
  if (!commentId) {
    return next(new ApiError(400, "commentId is missing"));
  }
...
const getLikedVideos = asyncHandler(async (req, res) => {
  ...
  if (!likedVideos) {
    return next(new ApiError(400, "liked videos not found"));
  }
